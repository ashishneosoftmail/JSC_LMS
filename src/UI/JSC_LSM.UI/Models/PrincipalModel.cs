...
namespace JSC_LSM.UI.Models
{
    public class PrincipalModel : CreatePrincipalDto
    {
        ...
        [Remote("CheckEmailExists", "Principal", HttpMethod = "POST", ErrorMessage = "EmailId already exists in database.")]

        public string EmailId { get; set; }
        [StringLength(100, ErrorMessage = "Username should not be more than 100 characters")]
        [Required(ErrorMessage = "Username is mandatory")]
        [Remote("CheckUserNameExists", "Principal", HttpMethod = "POST", ErrorMessage = "UserName already exists in database.")]
