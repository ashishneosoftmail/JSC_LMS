...
{
    public class GetAllParentsListBySchoolQueryHandler : IRequestHandler<GetAllParentsListBySchoolQuery, Response<IEnumerable<GetAllParentsListBySchoolDto>>>
    {
        ...
        public GetAllParentsListBySchoolQueryHandler(IMapper mapper, IStudentRepository studentRepository, IParentsRepository parentsRepository, IClassRepository classRepository, ISectionRepository sectionRepository, ISchoolRepository schoolRepository, IAuthenticationService authenticationService, ILogger<GetAllParentsListBySchoolQueryHandler> logger)
        {
        ...
        public async Task<Response<IEnumerable<GetAllParentsListBySchoolDto>>> Handle(GetAllParentsListBySchoolQuery request, CancellationToken cancellationToken)
        {
            _logger.LogInformation("Handle Initiated");

            var allParents = (await _parentsRepository.ListAllAsync()).Where<JSC_LMS.Domain.Entities.Parents>(x => x.SchoolId == request.SchoolId);
            ...
            List<GetAllParentsListBySchoolDto> parentsList = new List<GetAllParentsListBySchoolDto>();
            foreach (var parent in data)
            {

                parentsList.Add(new GetAllParentsListBySchoolDto()
                {
                    Id = parent.Id,
                    ParentName = parent.ParentName,
            ...
            return new Response<IEnumerable<GetAllParentsListBySchoolDto>>(parentsList, "success");
        }
    }
}
